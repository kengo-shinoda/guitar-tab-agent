...
class CandidatePositionsTest(unittest.TestCase):
    def test_low_e_has_one_open_position(self) -> None:
        ...
        self.assertEqual([(p.string_number, p.fret_number) for p in positions], [(6, 0)])

    def test_high_e_open_with_zero_max_fret(self) -> None:
        positions = candidate_positions_for_midi(64, max_fret=0)

        self.assertEqual([(p.string_number, p.fret_number) for p in positions], [(1, 0)])

    def test_pitch_can_have_multiple_positions(self) -> None:
        positions = candidate_positions_for_midi(64, max_fret=24)

        self.assertIn((1, 0), [(p.string_number, p.fret_number) for p in positions])
        self.assertIn((2, 5), [(p.string_number, p.fret_number) for p in positions])
        self.assertIn((3, 9), [(p.string_number, p.fret_number) for p in positions])
